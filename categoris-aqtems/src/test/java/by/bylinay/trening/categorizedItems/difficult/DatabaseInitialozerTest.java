...
class DatabaseInitialozerTest {
	...
	@Test
	public void testCheckingCreationOrReCreationDatabase()
	...
	public void testExeption() throws ClassNotFoundException, SQLException {
		DatabaseInitializer databaseInitializer = new DatabaseInitializer();
	...
	public static int getIdItem(String name) throws ClassNotFoundException, SQLException {
		ResultSet rs = ConnectorAndStatement.makeConnectionFndStatement()
				.executeQuery("select id from item where name_ = " + "'" + (name) + "'" + ";");
		int id = 0;
		while (rs.next()) {
			id = rs.getInt(1);
		}
	...
	public static int getIdCategory(String name) throws ClassNotFoundException, SQLException {
		ResultSet rs = ConnectorAndStatement.makeConnectionFndStatement()
				.executeQuery("select id from category where name_ = " + "'" + (name) + "'" + ";");
		int id = 0;
		while (rs.next()) {
			id = rs.getInt(1);
		}
